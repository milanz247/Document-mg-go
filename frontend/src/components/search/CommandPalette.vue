...
const items = computed<PaletteItem[]>(() => {
  ...
  if (auth.isAuthenticated) {
    actions.push(
      { id: 'new-page', label: 'Create new page', detail: 'Start a Markdown document', icon: FilePlus2, run: () => router.push('/editor/new') },
  ...
  const pages = flatten(documents.navigation)
    ...
    .map<PaletteItem>((node) => ({ id: `page:${node.path}`, label: node.name, detail: node.path, icon: FileText, run: () => router.push(documentRoute(node.path)) }))
